//! 압축파일을 더블클릭했을 때 뜨는 창. HexTool 검수를 마치고 "압축파일
//! 내보내기"를 누르면 생기는 이상현상 보고용 압축파일(FileKind::PhotoReport)
//! 전용이다 — 담긴 사진 장수만 보여준다(실제 내용물을 풀어 보여주는 기능은
//! 아직 없다).
...
impl App for ArchiveApp {
    ...
    fn update(&mut self, _ctx: &mut dyn RenderingBackend, r: &mut Renderer, assets: &Assets, area: Rect, win: &WinInput) -> AppAction {
        ...
        draw_icon(r, assets, &IconType::Archive, area.x + 14.0, area.y + 14.0, 40.0);

        // 창 폭이 좁아도 실제 폭 기준으로 줄바꿈해서 절대 안 잘리게 한다.
        let tx = area.x + 66.0;
        let text_w = (area.x + area.w - tx - 10.0).max(20.0);
        let line1 = t(lang, s::REPORT_LINE1).replace("{n}", &self.count.to_string());
        let line2 = t(lang, s::REPORT_LINE2).replace("{app}", secrets::PHOTOS_APP_NAME);
